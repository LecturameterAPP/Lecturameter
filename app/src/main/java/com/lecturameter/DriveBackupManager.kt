...
object DriveBackupManager {
    ...
    private suspend fun findBackupFileId(drive: Drive, context: Context): String? = withContext(Dispatchers.IO) {
    ...
    suspend fun backupWithAccount(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val existingId = findBackupFileId(drive, context)
                if (existingId != null) {
                    // Actualiza contenido Y nombre (refleja la fecha de hoy).
                    // Siempre hay exactamente un archivo en el Drive del usuario.
    ...
    suspend fun restore(
        ...
        runCatching {
            ...
            val drive = buildDriveService(context, account)
            val fileId = findBackupFileId(drive, context)
