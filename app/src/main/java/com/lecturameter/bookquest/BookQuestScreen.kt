...
class BookCoverBridge(private val context: Context, private val onExit: (() -> Unit)? = null) {
    ...
    // ── v2.4 rework: contador de victorias (visible solo en el WebView) ──────
    private val mainPrefs by lazy {
        context.getSharedPreferences("lecturameter", Context.MODE_PRIVATE)
    }

    /** Nº de victorias acumuladas en Book Quest. */
    @JavascriptInterface
    fun getWins(): Int = try {
        mainPrefs.getInt("bookquest_wins", 0)
    } catch (_: Exception) { 0 }

    /** Suma una victoria y devuelve el total actualizado. */
    @JavascriptInterface
    fun addWin(): Int = try {
        val total = mainPrefs.getInt("bookquest_wins", 0) + 1
        mainPrefs.edit().putInt("bookquest_wins", total).apply()
        total
    } catch (_: Exception) { 0 }
}
    ...
    AndroidView(
        ...
        factory = { context ->
            ...
            WebView(context).apply {
                ...
                // Feedback 2.6: LOAD_NO_CACHE obligaba a re-descargar TODAS las portadas
                // remotas en cada partida (lento en móvil; portadas en blanco si la red
                // va justa). El HTML es local (file://), así que no hay riesgo de servir
                // una versión vieja del juego: solo cachean las imágenes.
                settings.cacheMode =
